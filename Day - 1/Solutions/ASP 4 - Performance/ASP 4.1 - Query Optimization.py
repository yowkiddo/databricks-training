...
# MAGIC
# MAGIC
# MAGIC Note the lack of a **Filter** and the presence of a **PushedFilters** in the **Scan**. The filter operation is pushed to the database and only the matching records are sent to Spark, which are counted in the subsequent statement. This can greatly reduce the amount of data that Spark needs to ingest.
...
# MAGIC
# MAGIC This means Spark had to read ALL the data from the database and cache it, and then scan it in cache to find the records matching the filter condition. This is illustrated in the count returned from the **`cached_df`** DataFrame.
